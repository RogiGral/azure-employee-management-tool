...
export async function findDiscoverWeeklyPlaylist(
  queueItem: QueueItem,
  ...
  const playlistSearchResultObjectList =
    await spotifyAuthService.searchPlaylists('Discover Weekly')

  const playlistSearchResultObject =
    playlistSearchResultObjectList.body.playlists.items.find(
      (playlist) => playlist.name === 'Discover Weekly'
    )

  const trackList = await spotifyAuthService.getPlaylistTracks(
    playlistSearchResultObject.id
  )
